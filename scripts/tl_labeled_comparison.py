...
# 1. SETUP & SEEDING (Seed 42: The Answer to Life, the Universe, and Everything)
SEED = 42
np.random.seed(SEED)

parser = argparse.ArgumentParser(description='High-Res ML vs TL Comparison (Labeled Strings)')
parser.add_argument('file_2017', help='Path to labeled 2017 dataset')
parser.add_argument('file_2018', help='Path to labeled 2018 dataset')
...
def load_and_binarize(path):
    ...
    # Standardize Label: Case-insensitive and strip spaces
    # Anything that is NOT 'benign' becomes 1 (Attack)
...
# Fit scaler based on the 2017 "Known" environment
scaler = StandardScaler().fit(X17)
X17_scaled = scaler.transform(X17)
X18_scaled = scaler.transform(X18)

# 3. PRE-TRAINING THE EXPERT (TL BASE)
print("Pre-training Expert model on 2017 labels...")
expert_2017 = xgb.XGBClassifier(n_estimators=100, random_state=SEED, eval_metric='logloss')
expert_2017.fit(X17_scaled, y17)

# 4. EXPERIMENT SETUP (0.0% to 1.0%)
percentages = np.linspace(0, 1.0, 11) # 11 steps gives 0.0, 0.1, 0.2 ... 1.0
f1_rf, f1_xgb, f1_tl = [], [], []
...
for p in percentages:
    if p == 0:
        # Traditional models have zero knowledge with zero data
        f1_rf.append(0.0)
        f1_xgb.append(0.0)
        # TL Model uses 2017 expert opinion even with zero 2018 training
        baseline_preds = expert_2017.predict(X_test)
        f1_tl.append(f1_score(y_test, baseline_preds))
        print(f"Data: {p:.1f}% | RF: 0.000 | XGB: 0.000 | TL: {f1_tl[-1]:.3f} (Baseline)")
        continue

    # Sample very small slices of 2018 data
    sample_size = max(1, int((p / 100) * len(X18)))
...
plt.plot(percentages, f1_rf, label='RF (From Scratch)', marker='o', color='#95a5a6', alpha=0.7)
plt.plot(percentages, f1_xgb, label='XGB (From Scratch)', marker='s', color='#27ae60', alpha=0.7)
plt.plot(percentages, f1_tl, label='TL Stacking (2017 Knowledge)', marker='^', color='#2980b9', linewidth=2.5)

plt.title("The 'Head Start' Effect: Transfer Learning vs. Scratch Models", fontsize=14)
...
plt.savefig('tl_labeled_comparison.png', dpi=300)
print("\nSuccess! Saved as 'tl_labeled_comparison.png'")
plt.show()