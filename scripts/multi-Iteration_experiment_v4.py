...
# Test set is the final 10% of 2018
test_size = 10000
...
for i in range(NUM_ITERATIONS):
    ...
    # Shuffle pool for this run
    idx = np.random.permutation(len(X18_pool))
    
    for n in n_points:
        if n == 0:
            # ZERO-SHOT: Use a calibration window (200 samples) to find the best 2017->2018 threshold
            cal_idx = idx[:200]
            ...
            if len(np.unique(train_y)) < 2: # Skip bad random samples
            ...
            # 3. STACKED ADAPTIVE TL (Logistic Stacking for Stability)
            leg_probs_train = expert.predict_proba(train_X)[:, 1].reshape(-1, 1)
            # We stack the Legacy Prob + the Raw Features
            X_train_plus = np.hstack([train_X, leg_probs_train])
            X_test_plus = np.hstack([test_X, leg_probs_test])
            
            # Logistic Regression is much more stable for n=10 than XGBoost
            tl_model = LogisticRegression(max_iter=500).fit(X_train_plus, train_y)
    ...
    if (i+1) % 10 == 0: print(f"Iteration {i+1}/50 complete.")
