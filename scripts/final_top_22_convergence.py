...
def clean_dataset(df):
    """
    Cleans NaNs and Infinities which are common in Flow_Bytes_s 
    and Flow_Pkts_s columns of CIC-IDS datasets.
    """
    # Replace infinite values with NaN
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    # Drop rows with NaN values in our feature set
    df.dropna(subset=top_22_features + ['Label'], inplace=True)
    # Ensure features are numeric (some might be objects due to 'Inf' strings)
    for col in top_22_features:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    # Final drop in case numeric conversion created new NaNs
    df.dropna(subset=top_22_features, inplace=True)
    return df
...
try:
    ...
    print("Cleaning data (removing NaNs and Infs)...")
    df17 = clean_dataset(df17)
    df18 = clean_dataset(df18)
    ...
    for attack in attacks:
        ...
        try:
            ...
            plt.plot(sizes, scores, marker='o', label=f"{attack} (Adaptive TL)")
        except Exception as e:
            print(f"Skipping {attack}: {e}")

    plt.axvline(x=7200, color='red', linestyle='--', label='7,200 Reliability Threshold')
    plt.title('Monte Carlo Convergence: Adaptive Transfer (Cleaned 2017/2018 Data)')
    plt.xlabel('Adaptation Samples (2018)')
    plt.ylabel('F1 Score')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('final_top_22_convergence.png')
    plt.show()
