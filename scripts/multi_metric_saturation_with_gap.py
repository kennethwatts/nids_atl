...
def run_multi_metric_study(train_csv, test_csv, iterations=30):
    ...
    for size in sample_sizes:
        ...
        for i in range(iterations):
            ...
            with torch.no_grad():
                ...
                # ADDED: Calculate absolute P-R Gap
                metrics_log[size]['gap'].append(abs(p[best_idx] - r[best_idx]))
    ...
    # Calculate Means (Updated to include 'gap')
    ...
    # ADDED: Plot P-R Gap as a dotted gray line
    plt.plot(sample_sizes, plot_data['gap'], color='gray', linestyle=':', label='P-R Gap', alpha=0.8)

    plt.title('Monte Carlo: Optimized Threshold Metrics & Stability (100-1000 Samples)')
    ...
    plt.savefig('multi_metric_saturation_with_gap.png', dpi=300)
    plt.show()

    # Print TikZ Coordinates for LaTeX (Updated to include GAP)
    print("\n" + "="*40)
    print("TIKZ COORDINATES FOR MULTI-METRIC GRAPH")
    print("="*40)
    for metric in ['f1', 'pr', 're', 'acc', 'gap']:
